...
3. **Styling** — **Tailwind utility classes in the markup for layout/spacing/responsive**, and **design tokens in `globals.css` `@theme`** for the palette/type vocabulary. Rules:
   - Never hard-code a hex value or font name in a component. Reference a token utility (`text-ink`, `bg-canvas`, `border-hairline`) generated from `@theme`.
   - **The three-radius rule:** every `border-radius` on the site is one of exactly three tokens — `--radius-control` (6px: buttons, chips, inputs), `--radius-panel` (12px: cards, panels), `--radius-pill`. No fourth value, ever (docs/design-language.md §0.2.0 tell #4).
   - **One dominant hue per scroll zone, blended continuously** (2026-07 cinematic makeover — supersedes the old "one solid field per section"): sections are transparent over the flowing `DitherField`, whose dominant hue shifts down the document; other brand hues wash the same view, so every crop carries several colours leaning purple/gold — never a rainbow (the field grounds toward deep purple). Text never sits on the raw field: it sits on a `.scrim` pool or a token-gradient panel, and every such pairing (incl. the worst-case field blend) is gated by `npm run check:contrast` (docs/design-language.md §0.2.1). Accent discipline is unchanged: one gold interactive accent per viewport; three radius tokens.
   - **One accent per viewport:** gold (`--color-accent`) is THE interactive accent — one primary action carries it per viewport (the CTA, or link hovers). Never two competing gold elements at rest; never crimson as a second button colour (docs/design-language.md §0.2.1 role hierarchy).
