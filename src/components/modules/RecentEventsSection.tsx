 ...
 * not the featured flag; fed by recentPastEventsQuery). Editorial hairline rows,
 * not a card grid: mono date rail · title · type tag, hairline rules doing the
 * grouping (docs/design-language.md §0.2.0 — no cards-for-everything). Rows are
 * de-energized "past" — muted ink, no accent — with a surface tint + title
 * weight shift on hover.
 */
    ...
    <section className="flex min-h-[75svh] items-center px-6 py-16">
      ...
      <ScrimPool className="mx-auto w-full max-w-6xl p-5 sm:p-7">
        ...
        {events.length === 0 ? (
          ...
          // Column cards (user direction): image-forward — each recent event is
          // a media panel + caption, three across, so the (placeholder) frames
          // carry real presence instead of thumbnail slivers.
          <Reveal stagger role="list" className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {events.map((event) => (
              ...
              >
                ...
                />
                <span className="flex items-baseline gap-3 font-mono text-mono-label uppercase text-ink-muted">
